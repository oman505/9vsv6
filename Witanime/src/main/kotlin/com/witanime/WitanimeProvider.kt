...
class WitAnime : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        suspend fun fetchUrl(url: String): String {
            return try {
                app.get(url, headers = mapOf("User-Agent" to userAgent), referer = mainUrl).text
            } catch (e: Exception) {
                ""
            }
        }
        ...
        fun findServerElements(html: String): List<Pair<String, String>> {
            ...
            serverRegex.findAll(html).forEach { m ->
                ...
                val sid = Regex("""data-server-id=["']([^"']+)["']""").find(tag)?.groupValues?.get(1)
                    ?: Regex("""data-url=["']([^"']+)["']""").find(tag)?.groupValues?.get(1)
        ...
        return try {
            ...
            if (serversList.isEmpty() && resourceRegistryObj != null) {
                try {
                    if (resourceRegistryObj is JSONObject) {
                        resourceRegistryObj.keys().forEach { key ->
                            serversList.add(key to "Server $key")
                        }
                    } else if (resourceRegistryObj is JSONArray) {
                        for (i in 0 until resourceRegistryObj.length()) {
                            serversList.add(i.toString() to "Server $i")
                        }
                    }
                } catch (e: Exception) {
                }
            ...
            supervisorScope {
                val tasks = serversList.map { (sid, label) ->
                            ...
                            try {
                                ...
                                if (link.isBlank()) {
                                    if (sid.startsWith("http")) {
                                        ...
                                        try {
                                            val decoded = String(android.util.Base64.decode(sid, android.util.Base64.DEFAULT))
                                            if (decoded.contains("http")) {
                                                link = decoded.substring(decoded.indexOf("http"))
                                            }
                                        } catch (e: Exception) {
                                        }
                                    }
                                ...
                                val finalLink =
                                    if (link.contains("yonaplay.net/embed.php?id=")) {
                                        if (link.contains("apiKey=")) link else "$link&apiKey=$FRAMEWORK_HASH"
                                    } else link
